...
### 矩形補正はスコアリングの足切りを通れない

`RectangularGuideContour.detectContour` は**ガイドの bounds を矩形にして返す**ため、
できあがる輪郭は必ずガイドを包み込みます。

一方スコアリングの足切りには `inside`（**候補の点がガイド内部にある割合**、下限 0.58）があり、
包み込む形を想定していません。測ると、きれいな正方形のなぞりで 0.486、
揺れのある実際のなぞりでは **0.0** になります。

つまり `bias` が -0.18 であること以前に、**矩形補正の候補はスコアリングに載りません。**
移植元と同じ挙動です（スコアリングの出力は 170 ケースで一致を確認済み）。

被写体マスクのように**ガイドより内側へ収まる**抽出器はこの足切りを通るため、
影響を受けるのは矩形補正と、同じく bounds ベースの色矩形だけだと考えられます。

直すなら `inside` の測り方（候補 ⊂ ガイド）を見直すことになり、
**他の抽出器のスコアにも影響します。** StarWindow 側と揃える判断が要ります。

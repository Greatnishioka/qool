...
///
/// **切り抜きのマスクも同じ保管庫に置いています。** 元画像と区別せず、
/// 参照されているかどうかだけで残すか消すかを決めます。
        ...
        for element in canvas.elements {
            usedIDs.formUnion(
                [
                    ...
                    element.imageSource?.assetID,
                    element.cutoutMask?.assetID
                ].compactMap(\.self)
            )
            ...
            for snapshot in element.unionSourceElements {
                usedIDs.formUnion(
                    [
                        ...
                        snapshot.imageSource?.assetID,
                        snapshot.cutoutMask?.assetID
                    ].compactMap(\.self)
                )
            }
        }
